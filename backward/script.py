...
def measure_experiment(iterations, clients, entries, read, write, duration, experiment):
    ...
    for ex in experiment:
        if ex == "clients": clients = range(1,clients)
        if ex == "entries": entries = range(1,entries)
        if ex == "read": read = range(1,read)
        if ex == "write": write = range(1,write)
        if ex == "duration": duration = range(1,duration)
                    ...
                    for wxt in dict_e["write"]:
                        for duration in dict_e["duration"]:
                            ...
                            for it in range(iterations):
                                print(
                                    f"Interation {it}: {envs['clients']} clients {envs['entries']} entries {envs['read']} rxt {envs['write']} wxt {envs['duration']} duration {it}"
                                )
...
def main():
    ...
    measure_experiment(2, 2, 5, 6, 4, 10, experiment) #usage iterations, clients, entries, read, write, duration, experiment
